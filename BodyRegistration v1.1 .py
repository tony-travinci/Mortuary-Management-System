...
NameEntry=Entry(LeftInLeftFrame,width=53,font=('times',12,'italic'),bd=2,textvariable=NameEntryVariable,relief='ridge')
...
def connect():
        ...
        cursor=cnx.cursor()
        cursor.execute('''CREATE TABLE IF NOT EXISTS deceased(name VARCHAR (30),idnumber VARCHAR (30),kinname VARCHAR (30),kinnumber VARCHAR (30),bodywash VARCHAR(10),embalming VARCHAR(10),VIP VARCHAR(10),Flowers VARCHAR(10),Other VARCHAR(10),Coffin VARCHAR(10),Hearse VARCHAR(10),Tent VARCHAR(10),PostMortem VARCHAR(10))''')
        cursor.execute("INSERT INTO deceased(name,idnumber,kinname,kinnumber,bodywash,embalming,VIP,Flowers,Other,Coffin,Hearse,Tent,PostMortem) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",(NameEntry.get(),IdNumberEntry.get(),NextOfKinEntry.get(),KinPhoneEntry.get(),BodyWashingEntry.get(),EmbalmingEntry.get(),VIPEntry.get(),FlowerEntry.get(),OtherEntry.get(),CoffinEntry.get(),HearseEntry.get(),TentEntry.get(),PostMortemEntry.get(),))
        cnx.commit()
        cursor.close()
        cnx.close
#-----------------------checkbutton value--------------
